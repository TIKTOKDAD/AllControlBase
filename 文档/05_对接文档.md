...
### 5.4 轨迹默认值配置 (TrajectoryDefaults)

控制器支持通过 `TrajectoryDefaults` 类全局配置轨迹的默认参数，避免每次创建轨迹时重复指定：
...
# 方式 2: 直接修改类属性
TrajectoryDefaults.dt_sec = 0.1              # 默认时间步长
TrajectoryDefaults.low_speed_thresh = 0.1    # 低速阈值
TrajectoryDefaults.default_confidence = 0.9  # 默认置信度
TrajectoryDefaults.default_frame_id = 'base_link'  # 默认坐标系

# 配置后，创建 Trajectory 时可省略这些参数
trajectory = Trajectory(
    header=Header(stamp=time.time(), frame_id='base_link'),
    points=points,
    velocities=None,
    # dt_sec, confidence 等使用 TrajectoryDefaults 中的值
    soft_enabled=False,
)
```

> **注意**: `initialize_default_components()` 会自动调用 `TrajectoryDefaults.configure(config)`，通常不需要手动配置。

### 5.5 控制循环暂停处理

控制器内置了长时间暂停检测和恢复机制，适用于调试、暂停恢复等场景：
...
config['system'] = {
    ...
    # 暂停检测配置
    'long_pause_threshold': 0.5,   # 中等暂停阈值 (秒)
...
| 暂停时长 | 处理方式 | 说明 |
|----------|----------|------|
| < 0.5s | 正常处理 | 使用实际时间间隔进行预测 |
| 0.5s ~ 2s | 多步预测 | 分多步进行 EKF 预测，提高精度 |
| > 2s | 重置 EKF | 重置状态估计器，避免累积误差 |
...
# 恢复运行 - 控制器自动检测暂停并处理
cmd = manager.update(odom, trajectory)  # 自动使用多步预测
```
...
class DiagnosticsCollector:
    ...
    def get_latest(self):
        """获取最新诊断 - 可从任意线程调用"""
        with self.lock:
            return self.latest_diag

collector = DiagnosticsCollector()

# 添加回调（线程安全）
manager.set_diagnostics_callback(collector.on_diagnostics)
...
def monitor_thread():
    while running:
        diag = collector.get_latest()
        if diag:
            print(f"State: {diag['state']}, MPC: {diag['mpc_success']}")
        time.sleep(1.0)
```

**线程安全保证:**
- `set_diagnostics_callback()` 使用内部锁保护回调列表
- 回调函数在控制循环线程中同步调用
- 多个回调函数按添加顺序依次执行
- 回调函数抛出异常不会影响控制循环
