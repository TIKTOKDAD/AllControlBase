...
├── compat/                  # 兼容层模块 (2 个文件)
│   ├── __init__.py          # 兼容层入口
│   └── ros_compat_impl.py   # ROS 兼容实现 (MockTF2BufferCore 等)
│
├── config/                  # 配置模块 (17 个文件)
│   ├── __init__.py          # 配置模块入口
...
│   ├── constants.py         # 全局常量定义 (EPSILON, DEFAULT_GRAVITY 等)
│   ├── exceptions.py        # 统一异常体系 (ControllerError 等)
...
│   ├── __init__.py
│   ├── main_window.py       # 主窗口
│   ├── data_source.py       # 数据源接口 (直接访问 ControllerManager)
│   ├── ros_data_source.py   # ROS 数据源 (已弃用，迁移到 controller_ros)
│   ├── models.py            # 数据模型定义
...
└── tests/                   # 测试模块 (17 个文件)
    ...
    ├── run_dashboard_mock.py        # Dashboard 模拟测试
    ├── fixtures/                    # 测试数据和模拟
    │   ├── __init__.py
    │   ├── test_data_generator.py   # 测试数据生成器
    │   └── mock_diagnostics.py      # 模拟诊断数据生成
    ├── test_core.py                 # 核心模块测试
...
| `system_config.py` | 系统基础配置 | `ctrl_freq`, `watchdog`, `diagnostics` |
...
注意: 物理常量 (如重力加速度 `DEFAULT_GRAVITY`) 定义在 `core/constants.py` 中，不可配置。
...
| `constants.py` | 全局常量 | `EPSILON`, `EPSILON_SMALL`, `DEFAULT_GRAVITY`, `NEVER_RECEIVED_TIME_MS` |
| `exceptions.py` | 异常体系 | `ControllerError`, `ConfigurationError`, `ConfigValidationError` |
| `validators.py` | 数据验证 | 输入数据验证工具函数 |
...
| 类别 | 文件数 | 说明 |
|------|--------|------|
| 兼容层 | 2 | ros_compat_impl, __init__ |
| 配置模块 | 17 | system, mpc, safety, ekf, attitude, modules, platform, backup, consistency, trajectory, transform, transition, mock, validation, default, README, __init__ |
| 核心模块 | 11 | data_types, enums, interfaces, ros_compat, velocity_smoother, diagnostics_input, logging_config, constants, exceptions, validators, __init__ |
...
| Dashboard | 22 | main_window, data_source, ros_data_source, models, styles, 14 panels, 3 widgets |
| 消息定义 | 6 | 3 msg 文件, CMakeLists, package.xml, __init__ |
| 测试 | 17+ | 15 测试文件, run_all_tests, run_dashboard_mock, fixtures/ |
| 根目录 | 4 | __init__, main, README, requirements |
| **总计** | **~95** | 不含 __pycache__ |
