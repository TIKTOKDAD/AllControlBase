...
处理流程:
  ...
  2. StateEstimator.predict() ───> EKF 预测
  ...
  9. StateMachine.update() ───────> 状态转换
  10. SafetyMonitor.check() ──────> 安全检查
...
- `NORMAL → MPC_DEGRADED`: MPC 健康警告或 TF2 临界
...
```python
alpha = (
    kappa_consistency ** w_kappa *
    v_dir_consistency ** w_velocity *
    temporal_smooth ** w_temporal
) ** (1 / total_weight) * confidence
```
...
def update(odom, trajectory, imu=None) -> ControlOutput:
    # 1. 超时监控
    timeout_status = timeout_monitor.check(current_time)
...
class DashboardDataSource:
    def get_diagnostics() -> Dict      # 获取诊断数据
    def get_system_info() -> Dict      # 获取系统信息
    def get_statistics() -> Dict       # 获取运行统计
    def get_trajectory_data() -> Dict  # 获取轨迹数据
    def get_history() -> Dict          # 获取历史数据
```
