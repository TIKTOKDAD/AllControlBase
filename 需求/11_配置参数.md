...
DEFAULT_CONFIG = {
    ...
    'mpc': {
        ...
        'weights': {
            ...
            'velocity': 1.0,                # 速度跟踪权重
            'heading': 5.0,                 # 航向跟踪权重
            'control_v': 0.1,               # 速度控制量权重
            'control_omega': 0.1,           # 角速度控制量权重
        },
        'health_monitor': {
...
| `mpc.weights.control_v` | 0.1 | 速度控制量权重 |
| `mpc.weights.control_omega` | 0.1 | 角速度控制量权重 |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `watchdog.odom_timeout_ms` | 200 | Odom 超时阈值 (ms) |
| `watchdog.traj_timeout_ms` | 200 | 轨迹超时阈值 (ms) |
| `watchdog.traj_grace_ms` | 100 | 轨迹超时后的宽限期 (ms) |
| `watchdog.imu_timeout_ms` | 100 | IMU 超时阈值 (ms) |
| `watchdog.startup_grace_ms` | 1000 | 启动宽限期 (ms)，期间不判定超时 |
